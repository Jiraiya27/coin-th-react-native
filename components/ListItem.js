...
export default class CurrencyItem extends React.Component {
  ...
  render() {
    ...
    const volume = volume_24hours + " " + secondary_currency
    const latestPrice = formatNumber(last_price) + " " + getCurrencyUnit(primary_currency)
    ...
    return (
      <TouchableHighlight onPress={this.onPress}>
        <View style={styles.container}>
          ...
          <View style={styles.childView}>
            <Text adjustsFontSizeToFit numberOfLines={1} >{volume}</Text>
          </View>
          <View style={styles.childView}>
